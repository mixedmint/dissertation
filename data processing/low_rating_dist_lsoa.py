...
df = pd.read_csv("data processing/4_FHRS_agg_LSOA.csv", encoding="utf-8-sig")

total   = len(df)
n_zero  = (df["low_rating_pct"] == 0).sum()
pct_zero = n_zero / total * 100

print(f"LSOA 总计：{total}")
...
fig.suptitle("Distribution of Low-Rated Outlets (LSOA Level)",
...
axes[2].set_title(f"Restaurant Count per LSOA\n(n = {total:,})", fontsize=11)

for ax in axes:
    ax.spines[["top", "right"]].set_visible(False)

plt.tight_layout()
plt.savefig("data processing/low_rating_dist_lsoa.png", dpi=200, bbox_inches="tight")
plt.close()
print("\n已保存：data processing/low_rating_dist_lsoa.png")
