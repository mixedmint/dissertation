...
# ── 选取诊断变量 ──
# 参照组：ethnicity=White+Mixed_or_Multiple，business_type=restaurant_cafe_canteen（已删去）
# age：合并 0-4 和 65+ 为 Dependency_Ratio
...
control_cols   = ethnicity_cols + business_type_cols + ["Population_density"]
...
corr.to_csv("NBR/no_age/8_correlation_matrix_no_age.csv", encoding="utf-8-sig")
...
plt.tight_layout()
plt.savefig("NBR/no_age/8_correlation_heatmap_no_age.png", dpi=150)
plt.close()
print("热力图已保存：NBR/no_age/8_correlation_heatmap_no_age.png")

# ── 2. VIF ──
print("\n" + "=" * 60)
print("VIF（方差膨胀因子）")
print("=" * 60)
print("参照组：ethnicity=White+Mixed_or_Multiple，business_type=restaurant_cafe_canteen（已删去）\n")
...
vif_data.to_csv("NBR/no_age/8_vif_no_age.csv", index=False, encoding="utf-8-sig")

print("\n结果已保存：8_correlation_matrix_no_age.csv、8_vif_no_age.csv")
